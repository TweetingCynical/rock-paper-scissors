...
do {
  ...
  // User makes a choice
  userChoice = prompt(
    "What is your choice? Enter rock, paper or scissors."
  ).toLowerCase();
  // While loop if user enters choice incorrectly
  while (!gameOptions.includes(userChoice)) {
    userChoice = prompt(
      "Make sure you enter your choice correctly: Enter rock, paper or scissors."
    ).toLowerCase();
  }
  ...
  if (userChoice === cpuChoice) {
  ...
  } else if ( // Check whether user wins the round
    ...
    userPoints++;
    alert(
      `Congratulations, the computer chose ${cpuChoice}. You won that round!`
    );
  } else {
